...
export default function SajuResult() {
  ...
  const birthTimeUpdateMutation = useMutation({
    mutationFn: async (timeCode: string) => {
      ...
      const solarTerms = await getSolarTermsForCalculation(existingRecord.birthYear);
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <div className="flex items-center gap-0.5 mr-0">
          ...
          <Button 
            ...
            className="hover-elevate active-elevate-2 flex items-center gap-0.5 h-7 px-2"
          >
            <Edit className="h-2 w-2" />
            <span className="text-[0.5625rem]">수정</span>
          </Button>
          <Button 
            variant="default" 
            size="sm"
          ...
          >
            ...
            <span className="text-[0.5625rem]">{saveMutation.isPending ? "저장중..." : "저장"}</span>
          </Button>
        </div>
      </div>
